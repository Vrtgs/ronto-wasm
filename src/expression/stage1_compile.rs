use crate::expression::definitions::{
    BlockType, ControlFlowInstruction, Instruction, ParametricInstruction, SimpleInstruction,
};
...
pub(crate) fn compile(
    ...
    active_compilation: &mut ActiveCompilation,
) -> anyhow::Result<WasmVec<ResolvedInstruction>> {
    ...
    for instruction in expression.instructions.into_iter() {
        ...
        match instruction {
            Instruction::ControlFlow(instr) => match instr {
                ...
                ControlFlowInstruction::Else => {
                    let Some(IncompleteCf {
                        ty: bt @ IncompleteCfType::If,
                        ..
                    }) = control_flow.last_mut()
                    else {
                        unreachable!(
                            "there has to be an if as the last control flow; otherwise the expression would fail to decode"
                        )
                    };
                ...
                ControlFlowInstruction::End => {
                    ...
                    let complete_cf = match cf.ty {
                        ...
                        IncompleteCfType::IfElse { else_start } => StructuredControlFlow::IfElse {
                            start: cf.start,
                            else_start,
                            end,
            ...
            Instruction::Parametric(instr) => instructions.push(
                IncompleteResolveInstruction::Literal(LiteralInstruction::Parametric(instr)),
            ),
            Instruction::Simple(instr) => instructions.push(IncompleteResolveInstruction::Literal(
                LiteralInstruction::Simple(instr),
            )),
        }
    }

    Ok(WasmVec::from_trusted_box(
        instructions
            .into_iter()
